...
public class Advisor extends ChessPiece {
    ...
    public Vector<Integer> choosePiecePosition(JButton top, JButton right, JButton bottom, JButton left) {
        ...
        if (locateX < 5 && locateY > _top && !check.isEmpty(locateX + 1, locateY - 1) && pieces.elementAt(check.getPiece(locateX + 1, locateY - 1))._COLOR_ != this._COLOR_) {
            choose.add(pieces.elementAt(check.getPiece(locateX + 1, locateY - 1)).TYPE);
            pieces.elementAt(check.getPiece(locateX + 1, locateY - 1)).changeImage();
        }
        if (locateX > 3 && locateY > _top && !check.isEmpty(locateX - 1, locateY - 1) && pieces.elementAt(check.getPiece(locateX - 1, locateY - 1))._COLOR_ != this._COLOR_) {
            choose.add(pieces.elementAt(check.getPiece(locateX - 1, locateY - 1)).TYPE);
            pieces.elementAt(check.getPiece(locateX - 1, locateY - 1)).changeImage();
        }
        if (locateX > 3 && locateY < _down && !check.isEmpty(locateX - 1, locateY + 1) && pieces.elementAt(check.getPiece(locateX - 1, locateY + 1))._COLOR_ != this._COLOR_) {
            choose.add(pieces.elementAt(check.getPiece(locateX - 1, locateY + 1)).TYPE);
            pieces.elementAt(check.getPiece(locateX - 1, locateY + 1)).changeImage();
        }
        return choose;
