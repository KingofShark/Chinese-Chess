...
//        Nước đi của tượng sẽ không hợp lệ khi có một quân cờ nằm chặn giữa đường đi.
        ...
        if((_COLOR_ == BLACK)){
            ...
            if (locateY - 1 > 5 && locateX - 1 > 0 && check.checkLocal(locateX - 2, locateY - 2) && check.checkLocal(locateX - 1, locateY - 1)) {
                left.setLocation((locateX - 1) * CELL_SIZE - RADIUS, (locateY - 1) * CELL_SIZE - RADIUS);
                left.setVisible(true);//tl
            }
            if (locateY - 1 > 5 && locateX + 1 < 8 && check.checkLocal(locateX + 2, locateY - 2) && check.checkLocal(locateX + 1, locateY - 1)) {
                top.setLocation((locateX + 3) * CELL_SIZE - RADIUS, (locateY - 1) * CELL_SIZE - RADIUS);
                top.setVisible(true);//tr
            }
            ...
            //
            if (locateY - 1 > 5 && locateX - 1 > 0 && !check.checkLocal(locateX - 2, locateY - 2) && check.checkLocal(locateX - 1, locateY - 1) && pieces.elementAt(check.getPiece(locateX - 2, locateY - 2))._COLOR_ != _COLOR_) {
                chooses.add(pieces.elementAt(check.getPiece(locateX - 2, locateY - 2)).TYPE);
                pieces.elementAt(check.getPiece(locateX - 2, locateY - 2)).changeImage();
            }
            if (locateY + 1 > 5 && locateX + 1 < 8 && !check.checkLocal(locateX + 2, locateY - 2) && check.checkLocal(locateX + 1, locateY - 1) && pieces.elementAt(check.getPiece(locateX + 2, locateY - 2))._COLOR_ != _COLOR_) {
                chooses.add(pieces.elementAt(check.getPiece(locateX + 2, locateY - 2)).TYPE);
                pieces.elementAt(check.getPiece(locateX + 2, locateY - 2)).changeImage();
            }
